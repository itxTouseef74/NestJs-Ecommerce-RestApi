...
export class OrdersService {
  ...
  async create(createOrderDto: CreateOrderDto, currentUser: UserEntity): Promise<{ order: OrderEntity; clientSecret: string }> {
    const shippingEntity = new ShippingEntity();
    Object.assign(shippingEntity, createOrderDto.shippingAddress);
    const orderEntity = new OrderEntity();
    orderEntity.shippingAddress = shippingEntity;
    orderEntity.user = currentUser;
    orderEntity.amount = createOrderDto.amount;
    orderEntity.currency = createOrderDto.currency;
    ...
    for (let i = 0; i < createOrderDto.orderedProducts.length; i++) {
      ...
      opEntity.push({ order, product_quantity, product_unit_price, product });
    }

    await this.opRepository.createQueryBuilder().insert().into(OrdersProductsEntity).values(opEntity).execute();

    const paymentIntent = await this.paymentService.createPaymentIntent(createOrderDto.amount, createOrderDto.currency ,   createOrderDto.paymentMethodId , createOrderDto.returnUrl);
  ...
  async updateOrderStatusByPaymentIntentId(paymentIntentId: string, status: string) {
    const order = await this.orderRepository.findOne({ where: { paymentIntentId } });

    if (!order) {
      throw new NotFoundException('Order not found for the given payment intent ID');
    }
